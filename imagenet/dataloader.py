...
class ImagenetCounterfactual(Dataset):
    ...
    def __init__(self, ims_path, train=True, n_data=None, mode='x_gen'):
        ...
        # Transforms
        if train:
            t_list = [transforms.RandomResizedCrop(crop_from_to[1]), transforms.RandomHorizontalFlip()]
        else:
            t_list = [transforms.Resize(crop_from_to[0]), transforms.CenterCrop(crop_from_to[1])]
    ...
    def __init__(self):
        ...
        self.T = transforms.Compose(
            [transforms.Resize(crop_from_to[0]),
             transforms.CenterCrop(crop_from_to[1]),
             transforms.ToTensor(),
             transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                  std=[0.229, 0.224, 0.225])])
